...
# ── Passwordless sudo for startup_protect.sh ─────────────────────────────
# The startup script runs several commands with sudo from a desktop autostart;
# these sudoers rules let it run without prompting for a password.
WORK_DIR="$(pwd)"
REAL_USER="${SUDO_USER:-$USER}"
...
# Passwordless sudo for Chromecast Security Blocker startup scripts
# Generated by install.sh — edit carefully, validate with visudo -c
$REAL_USER ALL=(ALL) NOPASSWD: /usr/bin/nmcli con up chromecast-hotspot
$REAL_USER ALL=(ALL) NOPASSWD: $SYS_PY $WORK_DIR/chromecast_blocker.py
$REAL_USER ALL=(ALL) NOPASSWD: $SYS_PY $WORK_DIR/advanced_blocker.py
$REAL_USER ALL=(ALL) NOPASSWD: $SYS_PY $WORK_DIR/status_monitor.py
$REAL_USER ALL=(ALL) NOPASSWD: $VENV_PY $WORK_DIR/chromecast_blocker.py
$REAL_USER ALL=(ALL) NOPASSWD: $VENV_PY $WORK_DIR/advanced_blocker.py
$REAL_USER ALL=(ALL) NOPASSWD: $VENV_PY $WORK_DIR/status_monitor.py
SUDOERS

chmod 0440 "$SUDOERS_FILE"
# Validate syntax before keeping the file
if visudo -c -f "$SUDOERS_FILE" &>/dev/null; then
    echo "Sudoers rule installed → $SUDOERS_FILE"
    echo "  Startup will no longer prompt for a password."
else
    echo "[WARN] sudoers syntax check failed — removing $SUDOERS_FILE"
    rm -f "$SUDOERS_FILE"
fi
